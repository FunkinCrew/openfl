...
class TextureBase extends EventDispatcher
{
	...
	@:noCompletion private var __framebuffer:Dynamic;
	...
	@:noCompletion private var __textureID:Dynamic;
	...
	@:noCompletion private function new(context:Context3D, ?format:Context3DTextureFormat)
	{
		...
		if (__context.isBGFX)
		{
			...
			if (__supportsBGRA == null)
			{
				...
				else
				{
					...
					__textureInternalFormat = BGFXTextureFormat.RGBA8;
				}
			}
			...
			else
			{
				...
				{
					case RGB:
						__internalFormat = BGFXTextureFormat.RGB8;
						__format = BGFXTextureFormat.RGB8;
					case BGRA:
						__internalFormat = __textureInternalFormat;
						__format = __textureFormat;
					case RGBA:
						__internalFormat = BGFXTextureFormat.RGBA8;
						__format = BGFXTextureFormat.RGBA8;
					case R:
						__internalFormat = BGFXTextureFormat.R8;
						__format = BGFXTextureFormat.R8;
				}
			}
		}
		else
		{
			...
			{
				__supportsBGRA = true;
				__textureFormat = OpenGLRenderer.__bgraExtension.BGRA_EXT;
				__textureInternalFormat = OpenGLRenderer.__bgraAsInternalFormat ? OpenGLRenderer.__bgraExtension.BGRA_EXT : gl.RGBA;
			}
			else
			{
				__supportsBGRA = false;
		...
		if (__context.isBGFX)
		{
			...
			if (__framebuffer == null)
			{
				if (__textureID == null || __textureFlags & bgfx.TEXTURE_RT == 0)
				{
					...
					__textureFlags = bgfx.TEXTURE_RT | bgfx.TEXTURE_BLIT_DST;

					__textureID = bgfx.createTexture2D(__width, __height, false, 1, __internalFormat, __textureFlags, null);
				}
