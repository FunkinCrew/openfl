...
/**
	...
	and AIR.
**/
@:access(openfl.display3D.backends.bgfx.BGFXContext3D)
@:access(openfl.display3D.backends.opengl.OpenGLContext3D)
@:access(openfl.display3D.IContext3D)
...
{
	@:noCompletion private var __backend:IContext3D;
	...
	@:noCompletion private var __vertexConstants:openfl.utils._internal.Float32Array;
	@:noCompletion private var __fragmentConstants:openfl.utils._internal.Float32Array;
	...
	@:noCompletion public var __frameId:Int = 0;
	@:noCompletion public var __buffersReset:Array<Void->Void> = [];

	@:noCompletion private var __usingComplexBlend:Bool;

	@:noCompletion public var __blendTexture:TextureBase;
	...
	@:noCompletion public function new(stage:Stage, contextState:Context3DState = null, stage3D:Stage3D = null)
	{
		super();
		...
		{
			case BGFX:
				__backend = new openfl.display3D.backends.bgfx.BGFXContext3D(this, stage, contextState, stage3D);
			case OPENGL, OPENGLES, WEBGL:
				__backend = new openfl.display3D.backends.opengl.OpenGLContext3D(this, stage, contextState, stage3D);
			default:
				throw "Context3D is not supported on the current context backend";
		}
	}

	/**
	 * Defines wether this context is a `BGFX` context.
	 */
	public var isBGFX(get, never):Bool;

	/**
	 * Defines wether this context is an `OpenGL` context.
	 */
	public var isOpenGL(get, never):Bool;

	public var gl(get, never):WebGL2RenderContext;

	@:noCompletion private inline function get_gl()
		return __backend.gl;

	public var bgfx(get, never):BGFXRenderContext;

	@:noCompletion private inline function get_bgfx()
		return __backend.bgfx;
	...
	**/
	...
	@:noCompletion private inline function get_totalGPUMemory():Float
		return __backend.totalGPUMemory;
	...
	**/
	...
	@:noCompletion private function __clear(useScissor:Bool, red:Float = 0, green:Float = 0, blue:Float = 0, alpha:Float = 1, depth:Float = 1,
		...
		__backend.__clear(useScissor, red, green, blue, alpha, depth, stencil, mask);
	...
	**/
	public function createProgram(format:Context3DProgramFormat = AGAL):Program3D
		return __backend.createProgram(format);
	...
	**/
	public function createVertexBuffer(numVertices:Int, data32PerVertex:Int, bufferUsage:Context3DBufferUsage = STATIC_DRAW):VertexBuffer3D
		return __backend.createVertexBuffer(numVertices, data32PerVertex, bufferUsage);
	...
	**/
	public function createIndexBuffer(numIndices:Int, bufferUsage:Context3DBufferUsage = STATIC_DRAW):IndexBuffer3D
		return __backend.createIndexBuffer(numIndices, bufferUsage);
	...
	**/
	public function createTexture(width:Int, height:Int, format:Context3DTextureFormat, optimizeForRenderToTexture:Bool, streamingLevels:Int = 0):Texture
		return __backend.createTexture(width, height, format, optimizeForRenderToTexture, streamingLevels);
	...
	**/
	public function createRectangleTexture(width:Int, height:Int, format:Context3DTextureFormat, optimizeForRenderToTexture:Bool):RectangleTexture
		return __backend.createRectangleTexture(width, height, format, optimizeForRenderToTexture);
	...
	**/
	public function createCubeTexture(size:Int, format:Context3DTextureFormat, optimizeForRenderToTexture:Bool, streamingLevels:Int = 0):CubeTexture
		return __backend.createCubeTexture(size, format, optimizeForRenderToTexture, streamingLevels);
	...
	**/
	public function createVideoTexture():VideoTexture
		return __backend.createVideoTexture();
	...
	**/
	public function createASTCTexture(data:ByteArray):ASTCTexture
		return __backend.createASTCTexture(data);
	...
	**/
	public function isASTCSupported():Bool
		return __backend.isASTCSupported();

	/**
		Creates a new S3TCTexture instance from S3TC-compressed (DDS) data.
		...
		@throws IllegalOperationError If S3TC is not supported on this device.
		@throws IllegalOperationError If the DDS signature in `data` is invalid.
		@throws IllegalOperationError If the S3TC block format (DXT1/DXT3/DXT5) is not supported.
	**/
	public function createS3TCTexture(data:ByteArray):S3TCTexture
		return __backend.createS3TCTexture(data);

	/**
		Checks whether S3TC (DXT1, DXT3, DXT5) is supported on this Context3D instance.

		@return `true` if S3TC textures can be used on this device, `false` otherwise.
	**/
	public function isS3TCSupported():Bool
		return __backend.isS3TCSupported();

	/**
		Creates a MultiBufferTexture object, backed by one render target per requested format.

		@param width The width of each buffer, in pixels.
		@param height The height of each buffer, in pixels.
		@param formats The texture format of each buffer; the first is the primary attachment.
		@return A `MultiBufferTexture` ready for use as a render target.
	**/
	public function createMultiBufferTexture(width:Int, height:Int, formats:Array<Context3DTextureFormat>):MultiBufferTexture
		return __backend.createMultiBufferTexture(width, height, formats);
	...
	**/
	public function setProgramConstantsFromByteArray(programType:Context3DProgramType, firstRegister:Int, numRegisters:Int, data:ByteArray,
		...
		__backend.setProgramConstantsFromByteArray(programType, firstRegister, numRegisters, data, byteArrayOffset);
	...
	**/
	public function setProgramConstantsFromMatrix(programType:Context3DProgramType, firstRegister:Int, matrix:Matrix3D, transposedMatrix:Bool = false):Void
		__backend.setProgramConstantsFromMatrix(programType, firstRegister, matrix, transposedMatrix);
	...
	**/
	public function setProgramConstantsFromVector(programType:Context3DProgramType, firstRegister:Int, data:Vector<Float>, numRegisters:Int = -1):Void
		__backend.setProgramConstantsFromVector(programType, firstRegister, data, numRegisters);
	...
	**/
	public function setProgramConstantsFromArray(programType:Context3DProgramType, firstRegister:Int, data:Array<Float>, numRegisters:Int = -1):Void
		__backend.setProgramConstantsFromArray(programType, firstRegister, data, numRegisters);
	...
	**/
	public function setTextureAt(sampler:Int, texture:TextureBase):Void
		__backend.setTextureAt(sampler, texture);
	...
	**/
	public function setSamplerStateAt(sampler:Int, wrap:Context3DWrapMode, filter:Context3DTextureFilter, mipfilter:Context3DMipFilter):Void
		__backend.setSamplerStateAt(sampler, wrap, filter, mipfilter);
	...
	**/
	public function setRenderToTexture(texture:TextureBase, enableDepthAndStencil:Bool = false, antiAlias:Int = 0, surfaceSelector:Int = 0):Void
		__backend.setRenderToTexture(texture, enableDepthAndStencil, antiAlias, surfaceSelector);
	...
	**/
	public function setRenderToBackBuffer():Void
		__backend.setRenderToBackBuffer();
	...
	**/
	public function setBlendFactors(sourceFactor:Context3DBlendFactor, destinationFactor:Context3DBlendFactor):Void
		__backend.setBlendFactors(sourceFactor, destinationFactor);
	...
	@:dox(hide) @:noCompletion private function setBlendFactorsSeparate(sourceRGBFactor:Context3DBlendFactor, destinationRGBFactor:Context3DBlendFactor,
		...
		__backend.setBlendFactorsSeparate(sourceRGBFactor, destinationRGBFactor, sourceAlphaFactor, destinationAlphaFactor);
	...
	**/
	public function setColorMask(red:Bool, green:Bool, blue:Bool, alpha:Bool):Void
		__backend.setColorMask(red, green, blue, alpha);
	...
	**/
	public function setCulling(triangleFaceToCull:Context3DTriangleFace):Void
		__backend.setCulling(triangleFaceToCull);
	...
	**/
	public function setDepthTest(depthMask:Bool, passCompareMode:Context3DCompareMode):Void
		__backend.setDepthTest(depthMask, passCompareMode);
	...
	**/
	public function setScissorRectangle(rectangle:Rectangle):Void
		__backend.setScissorRectangle(rectangle);
	...
	**/
	public function setStencilActions(triangleFace:Context3DTriangleFace = FRONT_AND_BACK, compareMode:Context3DCompareMode = ALWAYS,
		...
		__backend.setStencilActions(triangleFace, compareMode, actionOnBothPass, actionOnDepthFail, actionOnDepthPassStencilFail);
	...
	**/
	...
	@:noCompletion public function __copyRenderTarget(target:TextureBase, width:Int, height:Int):Bool
	{
		...
		return cast(__backend, openfl.display3D.backends.bgfx.BGFXContext3D).__copyRenderTarget(target, width, height);
	}

	@:noCompletion public function __getRenderTargetSize():Point
	{
		if (!isBGFX) return new Point(backBufferWidth, backBufferHeight);

		return cast(__backend, openfl.display3D.backends.bgfx.BGFXContext3D).__getRenderTargetSize();
	}
	...
	@:noCompletion private inline function get___glDepthStencilFormat():Int
		return __backend.__glDepthStencilFormat;

	@:noCompletion private var __bgfxDepthFormat(get, never):Int;

	@:noCompletion private inline function get___bgfxDepthFormat():Int
		return __backend.__bgfxDepthFormat;

	@:noCompletion private var __maxTextureMaxAnisotropy(get, never):Int;

	@:noCompletion private inline function get___maxTextureMaxAnisotropy():Int
		return __backend.__maxTextureMaxAnisotropy;

	@:noCompletion private var __textureMaxAnisotropy(get, never):Int;

	@:noCompletion private inline function get___textureMaxAnisotropy():Int
		return __backend.__textureMaxAnisotropy;

	@:noCompletion private inline function get_isBGFX():Bool
		return __stage.window.context.type == BGFX;

	@:noCompletion private inline function get_isOpenGL():Bool
		return __stage.window.context.type == OPENGL || __stage.window.context.type == OPENGLES || __stage.window.context.type == WEBGL;
}
