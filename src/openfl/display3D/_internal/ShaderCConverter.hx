...
class ShaderCConverter
{
	...
	public function convertShaderSource(source:String, isVertex:Bool):String
	{
		...
		final globalConstReg = ~/^const\b/;
		final globalConstReplaceReg = ~/^(\s*)const\b/;
		...
		for (line in source.split('\n'))
		{
			...
			{
				var outLine = line;
				if (braceDepth == 0 && globalConstReg.match(trimmed))
				{
					usedOflConst = true;
					outLine = globalConstReplaceReg.replace(line, "$1_OFL_CONST");
				}

				results += outLine + '\n';
				braceDepth += (line.split("{").length - 1) - (line.split("}").length - 1);
				continue;
			}
		...
		if (usedOflConst) header += '#if BGFX_SHADER_LANGUAGE_HLSL \n#define _OFL_CONST static const\n#else\n#define _OFL_CONST const\n#endif\n';
