...
class GraphicsShader extends Shader
{
	...
	@:glFragmentHeader("varying float openfl_Alphav;
		...
		uniform sampler2D openfl_BlendTexture;
		uniform vec2 openfl_BlendTextureSize;
		uniform int openfl_BlendMode;

		const int OPENFL_BLEND_DARKEN = 2;
		const int OPENFL_BLEND_DIFFERENCE = 3;
		const int OPENFL_BLEND_HARDLIGHT = 5;
		const int OPENFL_BLEND_INVERT = 6;
		const int OPENFL_BLEND_LIGHTEN = 8;
		const int OPENFL_BLEND_OVERLAY = 11;
		const int OPENFL_BLEND_COLORDODGE = 15;
		const int OPENFL_BLEND_COLORBURN = 16;
		const int OPENFL_BLEND_SOFTLIGHT = 17;
		const int OPENFL_BLEND_EXCLUSION = 18;
		const int OPENFL_BLEND_HUE = 19;
		const int OPENFL_BLEND_SATURATION = 20;
		const int OPENFL_BLEND_COLOR = 21;
		const int OPENFL_BLEND_LUMINOSITY = 22;
		...
		vec3 openfl_setLum(vec3 colorBase, vec3 colorLum) {

			vec3 delta = colorLum - openfl_lum(colorBase);
			vec3 c = colorBase;
			c.r = c.r + delta.r;
			c.g = c.g + delta.g;
			c.b = c.b + delta.b;
		...
		vec3 openfl_blend(vec3 bg, vec3 src) {
			...
			if (openfl_BlendMode == OPENFL_BLEND_DARKEN) {
		...
		// Maybe move this to the CPU..
		bool openfl_hasBlendTexture() {

			return openfl_BlendMode == OPENFL_BLEND_DARKEN
				|| openfl_BlendMode == OPENFL_BLEND_DIFFERENCE
				|| openfl_BlendMode == OPENFL_BLEND_HARDLIGHT
				|| openfl_BlendMode == OPENFL_BLEND_INVERT
				|| openfl_BlendMode == OPENFL_BLEND_LIGHTEN
				|| openfl_BlendMode == OPENFL_BLEND_OVERLAY
				|| openfl_BlendMode == OPENFL_BLEND_COLORDODGE
				|| openfl_BlendMode == OPENFL_BLEND_COLORBURN
				|| openfl_BlendMode == OPENFL_BLEND_SOFTLIGHT
				|| openfl_BlendMode == OPENFL_BLEND_EXCLUSION
				|| openfl_BlendMode == OPENFL_BLEND_HUE
				|| openfl_BlendMode == OPENFL_BLEND_SATURATION
				|| openfl_BlendMode == OPENFL_BLEND_COLOR
				|| openfl_BlendMode == OPENFL_BLEND_LUMINOSITY;

		}

		vec4 openfl_applyBlend(vec4 src) {

			if (!openfl_hasBlendTexture()) return src;

			if (src.a <= 0.0) return src;

			vec4 bg = texture2D(openfl_BlendTexture, gl_FragCoord.xy / openfl_BlendTextureSize);

			vec3 res = openfl_blend(bg.rgb, src.rgb / src.a);

			return vec4(res * src.a, src.a);
