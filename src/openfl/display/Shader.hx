...
class Shader
{
	...
	**/
	...
	/**
		...
		This property is not available on the Flash target.
	**/
	public var shaderProgram(get, never):Dynamic;
	...
	**/
	...
	@:noCompletion private var __blendTexture:ShaderInput<BitmapData>;
	@:noCompletion private var __blendTextureSize:ShaderParameter<Float>;
	...
	**/
	...
	@:noCompletion private function __buildSourcePrefix(isFragment:Bool):String
	{
		...
		if (OpenGLRenderer.__drawBuffersEXT)
		{
			extensions += "#extension GL_EXT_draw_buffers : enable\n";
		}
		else if (OpenGLRenderer.__drawBuffersARB)
		{
		...
		if (__context != null && program == null)
		{
			...
			if (program != null)
			{
				...
				else if (__context.isBGFX)
				{
					for (input in __inputBitmapData)
					{
						var sampler = program.__bgfxSamplers.get(input.name);
						input.__bgfxSamplerID = (sampler != null && sampler.samplerID != null) ? sampler.samplerID : -1;
					}
					...
					for (parameter in __paramFloat)
					{
						...
						var attribIndex = program.__scc.getAttribIndex(parameter.name);
						if (attribIndex >= 0)
						{
							parameter.index = attribIndex;
							var useArrayUniform = program.__bgfxUniforms.get(parameter.name + "_useArray");
							if (useArrayUniform != null) parameter.__bgfxUseArrayUniform = useArrayUniform;
						}
					}
		...
		while (regex.matchSub(source, lastMatch))
		{
			...
			if (StringTools.startsWith(type, "sampler"))
			{
				...
				switch (name)
				{
					...
					case "bitmap":
						__bitmap = input;
					case "openfl_BlendTexture":
						__blendTexture = input;
					default:
				}
					...
					default:
						...
						if (StringTools.startsWith(name, "openfl_"))
						{
							switch (name)
							{
								...
								case "openfl_BlendTextureSize": __blendTextureSize = parameter;
								default:
							}
						}
	...
	@:noCompletion private function __updateGL():Void
	{
		...
		for (input in __inputBitmapData)
		{
			if (__context.isBGFX)
			{
				if (input.__bgfxSamplerID >= 0) input.__updateGL(__context, input.__bgfxSamplerID);
				continue;
			}
	...
	@:noCompletion private function __updateGLFromBuffer(shaderBuffer:ShaderBuffer, bufferOffset:Int):Void
	{
		...
		for (i in 0...shaderBuffer.inputCount)
		{
			...
			if (__context.isBGFX)
			{
				if (inputData != null && input.__bgfxSamplerID >= 0) input.__updateGL(__context, input.__bgfxSamplerID, inputData, inputFilter,
					inputMipFilter, inputWrap);
