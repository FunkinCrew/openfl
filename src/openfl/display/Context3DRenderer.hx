...
@:access(openfl.display.BGFXRenderer)
@:access(openfl.display.OpenGLRenderer)
@:access(openfl.display.IContext3DRenderer)
...
@:allow(openfl.text)
class Context3DRenderer extends DisplayObjectRenderer
{
	...
	@:noCompletion private static var __blendBitmapSize:Array<Float> = [0, 0];

	@:noCompletion private var __blendBitmap:BitmapData;
	...
	@:noCompletion private var __backend:IContext3DRenderer;
	...
	@:noCompletion private function new(context:Context3D, defaultRenderTarget:BitmapData = null)
	{
		...
		switch (context.__stage.window.context.type)
		{
			case BGFX:
				__backend = new BGFXRenderer(this);
			case OPENGL, OPENGLES, WEBGL:
				__backend = new OpenGLRenderer(this);
			default:
				throw "Context3DRenderer is not supported on the current context backend";
		}
	...
	**/
	...
	{
		if (__gl != null) __gl.viewport(__offsetX, __offsetY, __displayWidth, __displayHeight);
	}

	/**
		Setup the shader's blend modes
	**/
	@:noCompletion private function applyShaderBlend():Void
	{
		__shaderBlendModeValue = cast(__shaderBlendMode == null ? BlendMode.NORMAL : __shaderBlendMode);

		if (__currentShaderBuffer != null)
		{
			__currentShaderBuffer.addIntOverride("openfl_BlendMode", [__shaderBlendModeValue]);

			if (__shaderBlendMode != null) __currentShaderBuffer.addFloatOverride("openfl_BlendTextureSize", __blendBitmapSize);
		}
		else if (__currentShader != null)
		{
			if (__currentShader.__blendMode != null) __currentShader.__blendMode.value = [__shaderBlendModeValue];

			if (__shaderBlendMode != null && __currentShader.__blendTextureSize != null)
			{
				__currentShader.__blendTextureSize.value = __blendBitmapSize;
			}
		}
	}
	...
	**/
	...
	{
		if (__context3D.__state.renderToTexture != null || __defaultRenderTarget != null || __stage == null || __stage.__transparent)
		{
	...
	@:noCompletion private function __clearShader():Void
	{
		if (__currentShader != null)
		{
			...
			if (__currentShader.__blendTexture != null) __currentShader.__blendTexture.input = null;
			if (__currentShader.__blendTextureSize != null) __currentShader.__blendTextureSize.value = null;
		...
		if (__defaultRenderTarget == null)
		{
			...
			if (__offsetX > 0 || __offsetY > 0)
			{
				...
				if (__offsetX > 0)
				{
					...
					__context3D.__flush();
					__backend.__clearBackBufferGutter();
	...
	@:noCompletion private static function __isShaderBlend(value:BlendMode):Bool
	{
		return switch (value)
		{
			case DARKEN, DIFFERENCE, HARDLIGHT, INVERT, LIGHTEN, OVERLAY, COLORDODGE, COLORBURN, SOFTLIGHT, EXCLUSION, HUE, SATURATION, COLOR, LUMINOSITY: true;
			default: false;
		}
	}

	@:noCompletion private function __updateBlendTexture():Void
	{
		var size = __context3D.__getRenderTargetSize();
		var width = Std.int(size.x);
		var height = Std.int(size.y);
		...
		if (__blendBitmap == null || __blendBitmap.width != width || __blendBitmap.height != height)
		{
			if (__blendBitmap != null) __blendBitmap.dispose();
			__blendBitmap = BitmapData.fromTexture(__context3D.createRectangleTexture(width, height, BGRA, true), false);
		}

		var texture = __blendBitmap.getTexture(__context3D);
		if (!__context3D.__copyRenderTarget(texture, width, height))
		{
			__shaderBlendMode = null;
			__context3D.__blendTexture = null;
			return;
		}

		__context3D.__blendTexture = texture;
		__blendBitmapSize[0] = width;
		__blendBitmapSize[1] = height;
	}
	...
	@:noCompletion private override function __setBlendMode(value:BlendMode):Void
	{
		...
		if (!__complexBlendsSupported && __context3D.isBGFX && __isShaderBlend(value))
		{
			__blendMode = value;
			__shaderBlendMode = value;
			__updateBlendTexture();

			// shader blends should still render on screen with the normal blend!
	...
	@:noCompletion private function __setRenderTarget(renderTarget:BitmapData):Void
	{
		...
		__flipped = (renderTarget == null || !__backend.originBottomLeft);
	...
	@:noCompletion private function __updateShaderBuffer(bufferOffset:Int):Void
	{
		if (__currentShader != null && __currentShaderBuffer != null)
		{
			...
			__backend.__flushUseArray(__currentShader);
		}
	}
}
