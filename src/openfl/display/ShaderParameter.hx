...
@:final class ShaderParameter<T> /*implements Dynamic*/
{
	...
	**/
	...
	@:noCompletion private var __bgfxAttribBuffer:VertexBuffer3D;
	@:noCompletion private var __bgfxAttribData:Float32Array;
	@:noCompletion private var __bgfxUseArrayUniform:UniformData;
	@:noCompletion private var __bgfxUseArrayData:Float32Array;
	...
	@:noCompletion private function __updateGL(context:Context3D, overrideValue:Array<T> = null):Void
	{
		if (context.isBGFX)
		{
			...
			__setBGFXUseArray(context, false);
		}
	...
	@:noCompletion private function __updateGLFromBuffer(context:Context3D, buffer:Float32Array, position:Int, length:Int, bufferOffset:Int):Void
	{
		...
		{
			#if lime
			if (!__isUniform && index != null && length > __length)
			{
				var vertexCount = Std.int(length / __length);
				if (__bgfxAttribBuffer == null)
				{
					__bgfxAttribBuffer = context.createVertexBuffer(vertexCount, __length, Context3DBufferUsage.DYNAMIC_DRAW);
					__bgfxAttribBuffer.__buildSingleAttribLayout(index, __length);
				}

				if (__bgfxAttribData == null || __bgfxAttribData.length != length) __bgfxAttribData = new Float32Array(length);

				for (i in 0...length)
				{
					__bgfxAttribData[i] = buffer[position + i];
				}

				if (!__bgfxAttribBuffer.__uploadTransient(__bgfxAttribData))
				{
					__setBGFXUseArray(context, false);
					return;
				}

				var format = switch (__length)
				{
					...
					default: Context3DVertexBufferFormat.FLOAT_4;
				}
				context.setVertexBufferAt(index, __bgfxAttribBuffer, bufferOffset * __length, format);

				__setBGFXUseArray(context, true);
				return;
			}
	...
	@:noCompletion private inline function __setBGFXUseArray(context:Context3D, enabled:Bool):Void
	{
		__useArray = enabled;
		if (__bgfxUseArrayUniform != null)
		{
			if (__bgfxUseArrayData == null) __bgfxUseArrayData = new Float32Array(4);
			__bgfxUseArrayData[0] = enabled ? 1.0 : 0.0;
			context.bgfx.setUniform(__bgfxUseArrayUniform.uniform, __bgfxUseArrayData, __bgfxUseArrayUniform.info.num);
		}
	}
